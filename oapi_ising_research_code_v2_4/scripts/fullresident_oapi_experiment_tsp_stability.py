...
def phase_stability(args: argparse.Namespace, out: Path, target_map) -> None:
    """Mechanism reproduction: q-xi map for any supported full-resident family.

    ER/SK use M=N resident spins.
    TSP/planted-TSP use M=N_city^2 resident spins; no variable partitioning.

    For TSP, the CSV additionally stores feasibility and permutation-constraint
    diagnostics for the minimum-energy state found by each stochastic trajectory.
    """
    rows: List[Dict[str, Any]] = []
    ...
    supported = ("er", "sk", "signed_er", "planted", "tsp", "planted_tsp")

    for family in families:
        if family not in supported:
            raise ValueError(f"Unsupported stability family={family!r}; choose from {supported}")

        is_tsp = family in ("tsp", "planted_tsp")
        n_this = int(args.tsp_cities if is_tsp else args.n)
        p_this = float("nan") if is_tsp else float(args.p)

        for seed in seeds:
            problem = make_problem(
                family,
                n_this,
                args.p,  # ignored by TSP generators
                seed,
                args.tsp_A,
                args.tsp_B,
                args.normalize,
            ...
            md = problem.metadata or {}
            tsp_n_cities = int(md.get("n_cities", n_this)) if is_tsp else None
            tsp_D = np.asarray(md["D"], dtype=float) if is_tsp and "D" in md else None
            ...
            for qv in q_grid:
                for xiv in xi_grid:
                    ...
                    res = FullResidentPBitSolver(
                        problem,
                        cfg,
                        PBitTransfer(args.pbit_gain, args.pbit_bias),
                    ).run()

                    late = max(1, len(res.history["O"]) // 4)
                    row: Dict[str, Any] = {
                        "family": family,
                        # For TSP, n is the number of cities; n_spins is N_city^2.
                        "n": n_this,
                        "n_spins": int(problem.n),
                        "p": p_this,
                        "instance_seed": seed,
                        ...
                        "spectral_rho_D1_beta_max": spectral_radius_linearized(
                            Jnp, cfg.anneal.beta_max, qv, xiv
                        ),
                        **edge_info,
                    }

                    if is_tsp and tsp_D is not None:
                        dec = [
                            tsp_decode(
                                res.best_state[j],
                                tsp_n_cities,
                            ...
                            for j in range(len(res.best_energy))
                        ]
                        feasible = np.asarray(
                            [bool(x["feasible"]) for x in dec], dtype=bool
                        )
                        violation = np.asarray(
                            [float(x["constraint_violation"]) for x in dec], dtype=float
                        )
                        route_distance = np.asarray(
                            [
                                float(x["tour_distance"]) if bool(x["feasible"]) else np.nan
                                for x in dec
                            ],
                            dtype=float,
                        )
                        row.update({
                            ...
                            "tsp_feasible_rate": float(np.mean(feasible)),
                            "constraint_violation_mean": float(np.mean(violation)),
                            "constraint_violation_std": float(np.std(violation)),
                            "constraint_violation_min": float(np.min(violation)),
                            "best_route_distance_mean": (
                                float(np.nanmean(route_distance))
                                if np.any(np.isfinite(route_distance))
                                else np.nan
                            ),
                            "n_feasible_trajectories": int(np.sum(feasible)),
                            "tsp_A": float(args.tsp_A),
                            "tsp_B": float(args.tsp_B),
                        })
    ...
    if not rows:
        raise RuntimeError(
            "Stability phase produced no rows. Check --families and problem settings."
        )
