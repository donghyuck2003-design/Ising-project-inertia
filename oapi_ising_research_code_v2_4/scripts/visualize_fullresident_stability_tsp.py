...
fullresident_oapi_experiment.py --phase stability, including full-resident TSP sweeps.
...
Main outputs (per problem family):
  ...
  7) TSP feasible-rate / constraint-violation maps, when TSP columns are present
...
def clean_family_name(x: str) -> str:
    x = str(x).strip()
    return {"er": "Dense ER Max-Cut", "sk": "SK", "tsp": "Full-resident TSP", "planted_tsp": "Planted full-resident TSP"}.get(x.lower(), x.upper())
...
def prepare(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    numeric = [
        ...
        "tsp_feasible_rate", "constraint_violation_mean", "constraint_violation_std",
        "constraint_violation_min", "best_route_distance_mean", "n_feasible_trajectories",
    ]
    for c in numeric:
        if c in d.columns:
    ...
    if "tsp_feasible_rate" in d:
        agg_spec["tsp_feasible_rate"] = ("tsp_feasible_rate", "mean")
    if "constraint_violation_mean" in d:
        agg_spec["constraint_violation_mean"] = ("constraint_violation_mean", "mean")
    if "constraint_violation_min" in d:
        agg_spec["constraint_violation_min"] = ("constraint_violation_min", "mean")
    if "best_route_distance_mean" in d:
        agg_spec["best_route_distance_mean"] = ("best_route_distance_mean", "mean")
    if "n_feasible_trajectories" in d:
        agg_spec["n_feasible_trajectories"] = ("n_feasible_trajectories", "sum")
...
def write_findings(d: pd.DataFrame, agg: pd.DataFrame, path: Path) -> None:
    ...
    for family in sorted(d["family"].astype(str).unique()):
        ...
        if "tsp_feasible_rate" in a.columns and a["tsp_feasible_rate"].notna().any():
            best_feas = a.loc[a["tsp_feasible_rate"].idxmax()]
            lines.append(
                f"Highest TSP feasible rate: q={best_feas.q:g}, xi={best_feas.xi:g}, "
                f"feasible_rate={best_feas.tsp_feasible_rate:.6g}."
            )
        if "constraint_violation_mean" in a.columns and a["constraint_violation_mean"].notna().any():
            best_v = a.loc[a["constraint_violation_mean"].idxmin()]
            lines.append(
                f"Lowest mean TSP constraint violation: q={best_v.q:g}, xi={best_v.xi:g}, "
                f"violation={best_v.constraint_violation_mean:.6g}."
            )
...
def main() -> None:
    ...
    for family in sorted(agg["family"].astype(str).unique()):
        ...
        # TSP-specific diagnostic maps. These are the primary plots to inspect
        # before interpreting energy, because infeasible states are not valid tours.
        if "tsp_feasible_rate" in a.columns and a["tsp_feasible_rate"].notna().any():
            t = a.pivot(index="xi", columns="q", values="tsp_feasible_rate").sort_index().sort_index(axis=1)
            annotated_heatmap(
                t,
                f"{label}: feasible-tour rate",
                "Feasible-tour rate",
                out / f"{slug}_heatmap_feasible_rate",
                formats,
                args.dpi,
                args.annotate,
                ".3f",
            ...
            line_by_xi(
                ...
                "tsp_feasible_rate",
                "Feasible-tour rate",
                f"{label}: feasibility vs update parallelism",
                out / f"{slug}_q_vs_feasible_rate",
                formats,
                args.dpi,
            )

        if "constraint_violation_mean" in a.columns and a["constraint_violation_mean"].notna().any():
            t = a.pivot(index="xi", columns="q", values="constraint_violation_mean").sort_index().sort_index(axis=1)
            annotated_heatmap(
                t,
                f"{label}: permutation-constraint violation",
                "Mean constraint violation",
                out / f"{slug}_heatmap_constraint_violation",
                formats,
                args.dpi,
                args.annotate,
                ".2f",
            ...
            line_by_xi(
                ...
                "Mean constraint violation",
                f"{label}: constraint violation vs update parallelism",
                out / f"{slug}_q_vs_constraint_violation",
                formats,
                args.dpi,
            )

        if "best_route_distance_mean" in a.columns and a["best_route_distance_mean"].notna().any():
            t = a.pivot(index="xi", columns="q", values="best_route_distance_mean").sort_index().sort_index(axis=1)
            annotated_heatmap(
                t,
                f"{label}: feasible-route distance",
                "Mean route distance among feasible best states",
                out / f"{slug}_heatmap_route_distance",
                formats,
                args.dpi,
                args.annotate,
                ".3f",
