...
METHODS = ["par0", "fixed_pimi", "fixed_partial", "adamw_rms", "adaptive_q", "joint", "joint_restart"]


def main():
    ap = argparse.ArgumentParser(description="Phase C/D: adaptive-xi, adaptive-q, joint, restart ablation")
    ...
    ap.add_argument("--instance-seeds", default="20,21,22")
    ap.add_argument("--solver-seed", type=int, default=2000)
    ap.add_argument("--steps", type=int, default=3500); ap.add_argument("--batch", type=int, default=16)
    ap.add_argument("--fixed-xi", type=float, default=0.30); ap.add_argument("--fixed-q", type=float, default=0.50)
    ap.add_argument("--device", default="auto"); ap.add_argument("--out", default=str(ROOT / "results" / "phase_c_joint"))
    args = ap.parse_args()
    ...
    for iseed in [int(x) for x in args.instance_seeds.split(",")]:
        ...
        for mi, method in enumerate(METHODS):
            ...
            base.controller.q_fixed = args.fixed_q if method == "fixed_partial" else 1.0
    ...
    pd.DataFrame(rows).to_csv(out / "joint_ablation_runs.csv", index=False)
    print(f"Saved: {out / 'joint_ablation_runs.csv'}")
